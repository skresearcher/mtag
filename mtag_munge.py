...
def munge_sumstats(args, write_out=True, new_log=True):
    ...
    try:
        ...
        if (not args.n_value) and (not args.N) and (not (args.N_cas and args.N_con)) and ('N' not in list(cname_translation.values())) and\
        ...
        signed_sumstats = [k for k,v in list(cname_translation.items()) if v=='SIGNED_SUMSTAT']
